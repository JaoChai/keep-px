...
func (r *PixelRepo) GetByID(ctx context.Context, id string) (*domain.Pixel, error) {
	...
	err := r.pool.QueryRow(ctx,
		`SELECT id, customer_id, fb_pixel_id, fb_access_token, name, is_active, status, created_at, updated_at
		 FROM pixels WHERE id = $1`, id,
	).Scan(&p.ID, &p.CustomerID, &p.FBPixelID, &p.FBAccessToken, &p.Name, &p.IsActive, &p.Status, &p.CreatedAt, &p.UpdatedAt)
...
func (r *PixelRepo) ListByCustomerID(ctx context.Context, customerID string) ([]*domain.Pixel, error) {
	...
	for rows.Next() {
		p := &domain.Pixel{}
		if err := rows.Scan(&p.ID, &p.CustomerID, &p.FBPixelID, &p.FBAccessToken, &p.Name, &p.IsActive, &p.Status, &p.CreatedAt, &p.UpdatedAt); err != nil {
			return nil, err
		}
...
func (r *PixelRepo) Update(ctx context.Context, p *domain.Pixel) error {
	_, err := r.pool.Exec(ctx,
		`UPDATE pixels SET fb_pixel_id = $2, fb_access_token = $3, name = $4, is_active = $5, status = $6, updated_at = NOW()
		 WHERE id = $1`,
		p.ID, p.FBPixelID, p.FBAccessToken, p.Name, p.IsActive, p.Status,
	)
	return err
}
