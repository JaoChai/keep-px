...
func (r *ReplaySessionRepo) GetByID(ctx context.Context, id string) (*domain.ReplaySession, error) {
	...
	err := r.pool.QueryRow(ctx,
		`SELECT id, customer_id, source_pixel_id, target_pixel_id, status, total_events, replayed_events, failed_events, event_types, date_from, date_to, time_mode, batch_delay_ms, error_message, started_at, completed_at, created_at
		 FROM replay_sessions WHERE id = $1`, id,
	).Scan(&s.ID, &s.CustomerID, &s.SourcePixelID, &s.TargetPixelID, &s.Status, &s.TotalEvents, &s.ReplayedEvents, &s.FailedEvents, &s.EventTypes, &s.DateFrom, &s.DateTo, &s.TimeMode, &s.BatchDelayMs, &s.ErrorMessage, &s.StartedAt, &s.CompletedAt, &s.CreatedAt)
...
func (r *ReplaySessionRepo) ListByCustomerID(ctx context.Context, customerID string) ([]*domain.ReplaySession, error) {
	...
	for rows.Next() {
		s := &domain.ReplaySession{}
		if err := rows.Scan(&s.ID, &s.CustomerID, &s.SourcePixelID, &s.TargetPixelID, &s.Status, &s.TotalEvents, &s.ReplayedEvents, &s.FailedEvents, &s.EventTypes, &s.DateFrom, &s.DateTo, &s.TimeMode, &s.BatchDelayMs, &s.ErrorMessage, &s.StartedAt, &s.CompletedAt, &s.CreatedAt); err != nil {
			return nil, err
		}
