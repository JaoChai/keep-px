...
> ⚠️ **เอกสารประวัติศาสตร์ — ชุดเทสต์นี้ถูกลบทิ้งแล้วเมื่อ 2026-08-12**
> `frontend/e2e/` (31 spec), Playwright และ job `post-deploy-e2e` ถูกเอาออกทั้งหมดตามคำสั่งเจ้าของ
> เก็บไฟล์นี้ไว้เป็นบันทึกว่าเคยครอบคลุมอะไรบ้าง เผื่อวันหนึ่งอยากทำใหม่
> การทดสอบที่ยังเหลืออยู่: Go `go test ./...` · vitest ฝั่ง frontend · vitest ของ `worker/`
